...
| structural-complete regalloc miss over a re-materializable constant loop-invariant (`&arr[K]`) on the wrong side of a call boundary (callee-saved where the ROM wants caller-saved, or the reverse against an `&arr[i]` sibling), flipping a call-arg copy-pref; move its define point across the call + inline the sentinel | #loop-weight-and-live-length-regalloc-steering |
| prologue saves `$fp`, sets `addu $fp,$sp,$zero`, then `subu $sp,$sp,rX` where `rX = ((n*4)+14)>>3<<3`, plus an `lw 0($sp)` probe | #variable-length-array-dynamic-frame |
...
| byte-exact except a 3-word branch-direction triple on a `cond?t\|K:t` store/print (cracked: give `t` a later use) | #cse-make_regs_eqv-branch-fold |
...
| structural-complete regalloc miss = which value wins an earlier caller-saved reg (before "irreducible"), incl. a call-crossing param/local grabbing `$s0` and rotating the loop vars off `s0/s1/s2` (local-alloc pre-empts global priority; fix = `p=f(p)` in place to make it a global qty); diagnose with the `-dg`/`-dl` allocno dumps | #loop-weight-and-live-length-regalloc-steering |
| register-name-only permutation at the ROM's exact count and instruction order; need one allocno past another without emitting an instruction (`do {} while (0)` as a per-region ref multiplier) | #pervasive-regalloc-classical-main |
...
| a stub tagged `POOL-BLOCKED` (S300) | #duplicate-literal-pool-shared-rodata |
| isolated build byte-exact yet `verify-rom.sh` fails; every `D_<addr>` past a point placed at `addr+n` in the map, and the object contributes its own `.rodata` of size `n`: source-literal FP constants built a second pool beside the TU's blob; reference the blob entries `extern const`, one distinct symbol per textual occurrence | #duplicate-literal-pool-shared-rodata |
| exact count + identical multiset incl. registers/immediates, residual is pure instruction order in one window, surviving `-fno-schedule-insns2` (not a scheduler coin): emission order fixed inside an SDK macro (`gDPSetScissor` evaluates `pkt`, hence `gfx++`, before the coordinates) | #macro-internal-emission-order |
| guard branch byte-exact except its delay slot (ROM `nop`, build steals from the fall-through thread, sometimes over a load): `may_trap_p`/`own_thread_p`/`stop_search_p`; null-guard case turns on whether body-first derefs the ptr | #reorg-thread-availability-an-empty-rom-delay-slot-is-not-branch-prediction |
