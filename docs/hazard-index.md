...
| structural-complete regalloc miss over a re-materializable constant loop-invariant (`&arr[K]`) on the wrong side of a call boundary — callee-saved where the ROM wants caller-saved, or caller-saved against a variable-index `&arr[i]` sibling — flipping a call-arg copy-pref (mask→`$a0` vs `$t1`); move its define point across the call + inline the sentinel | #loop-weight-and-live-length-regalloc-steering |
...
| exact count + instruction multiset identical incl. registers/immediates, residual is pure instruction order in one window, and it survives `-fno-schedule-insns2` (so not a scheduler coin): emission order fixed inside an SDK macro (`gDPSetScissor` evaluates `pkt`, hence `gfx++`, before the coordinates); hoisting the bump out gets the order and loses the insn to DSE | #macro-internal-emission-order |
