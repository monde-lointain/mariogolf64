...
def test_pack_factories():
    assert H.pack(h.HAZARD_PACK, 3, ["a=foo", "b=?"]).render() == f"{h.HAZARD_PACK}:3fn[a=foo,b=?]"
    assert H.pack(h.HAZARD_SINGLE_FILE_PACK, 2, ["a=foo", "b=foo"]).render() \
        == f"{h.HAZARD_SINGLE_FILE_PACK}:2fn[a=foo,b=foo]"
    assert H.upstream_fncount_mismatch(3, 2).render() == f"{h.HAZARD_UPSTREAM_FNCOUNT_MISMATCH}:3vs2"
...
def test_scalar_factories():
    ...
    assert H.maybe_upstream("libultra", ["f1", "f2"]).render() == f"{h.HAZARD_MAYBE_UPSTREAM}:libultra:f1,f2"
...
def test_address_list_factories():
    assert H.rodata_jtbl([0x80001000, 0x80000000]).render() \
    ...
    assert H.needs_header(["h.h(adapt)"]).render() == f"{h.HAZARD_NEEDS_HEADER}:h.h(adapt)"
    assert H.caller_evict(["fn@a", "fn2@b,c"]).render() == f"{h.HAZARD_CALLER_EVICT}:fn@a;fn2@b,c"
...
def test_header_factories():
    assert H.header_renames_symbol("fn", "h.h").render() == f"{h.HAZARD_HEADER_RENAMES_SYMBOL}:fn@h.h"
    assert H.wrong_ghidra_name("g", "c", "h.h").render() == f"{h.HAZARD_WRONG_GHIDRA_NAME}:g->c@h.h"
    assert H.stale_header(["VERSION_K"]).render() == f"{h.HAZARD_STALE_HEADER}:os_version.h(VERSION_K)"


def test_coddog_factories():
    assert H.coddog_twin("foo", ["b", "a"]).render() == f"{h.HAZARD_CODDOG_TWIN}:foo!=a,b"
    assert H.coddog_fncount_mismatch(2, 3).render() == f"{h.HAZARD_CODDOG_FNCOUNT_MISMATCH}:2vs3"
    assert H.coddog_structural("f.c", 99.0).render() == f"{h.HAZARD_CODDOG_STRUCTURAL}:f.c@99.00"
    assert H.coddog_partial(2, 12).render() == f"{h.HAZARD_CODDOG_PARTIAL}:2of12fn"
    assert H.coddog_source_banked("rotate.c").render() == f"{h.HAZARD_CODDOG_SOURCE_BANKED}:rotate.c"
...
def test_game_region_mirror():
    assert H.game_region_mirror(0x800298E0, 0x298E0).render() == f"{h.HAZARD_GAME_REGION_MIRROR}:0x800298E0"
    assert H.game_region_mirror(None, 0x298E0).render() == f"{h.HAZARD_GAME_REGION_MIRROR}:0x298e0"
