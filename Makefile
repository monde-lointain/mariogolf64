...
# Vendored body-includes (e.g. the n_audio_sc `inc/*.inc` fragments `#include`d
# inside a function body) use the `.inc` extension, so `find -name '*.c'` never
# sweeps them: they are not standalone TUs and must not be compiled as their own
# objects.
