...
> **Provenance.** Vendored copy of `~/development/PROMPT_GUIDELINES.md` (Anthropic's prompt
> engineering guide), as-of 2026-06-17. Reduced to the general, model-agnostic guidance plus the
> dedicated `Prompting Claude Opus 4.8` section (the model this project runs). The other
> model-specific section (Claude Fable 5) and the cross-model migration guide were dropped. Re-sync
> the `Prompting Claude Opus 4.8` section wholesale from the upstream guide when Anthropic updates
> it. This is a managed copy, not authored from scratch; keep it verbatim so the re-sync stays
> mechanical.

This is the project's prompt-engineering reference. Every prompt surface in the repo (`CLAUDE.md`,
`docs/*`, `.claude/commands/*`, the artifact templates) follows it. It has two parts: general
techniques that apply across Claude's current models, then a dedicated `Prompting Claude Opus 4.8`
section covering behaviors specific to the model this project uses.
