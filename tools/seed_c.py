...
def stitch_base_c(spec: BaseCSpec) -> Path:
    ...
    # authority; the Ghidra decompile can be silently wrong. Embed it inline so
    # the agent translates from it, especially when the decompile is degenerate or
    # absent. See docs/hazards.md#decompile-vs-asm-authority.
    ...
    if ghidra_path.exists():
        ...
        if degenerate:
            ...
            lines.append("// shape-only, not to be trusted.")
        lines.append(body)
    else:
        lines.append(
            f"// TODO: no Ghidra decompile for {placeholder} — translate from the asm"
        )
        lines.append(
