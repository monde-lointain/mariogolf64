...
The seed is a combined m2c-body + Ghidra-typed-context seed: m2c translates the
asm into the compiled body, typed by a Ghidra-MCP struct context, and the Ghidra
decompile drops to a shape/type reference. See
docs/hazards.md#decompile-vs-asm-authority.

Pipeline:
  1. (--parent) m2ctx.py <parent.c> -> ctx.c          (parent type context)
  2. (ghidra_ctx.c) m2ctx.py <ghidra_ctx.c> -> ctx_ghidra.c   (RE'd struct context)
  3. extract_functions slice of <placeholder> from asm/<seg>.s -> target.s
  4. (best-effort) rodata slicing from asm/data/*.{rodata,data}.s -> target.rodata.s
  5. m2c -t mips-gcc-c [--context ...] target.s [target.rodata.s] -> m2c.c
  6. Read nonmatchings/<func>/ghidra.c if the slash command wrote one
  7. Stitch base.c: <ultra64.h> + DLIST_HINT + externs + ghidra/asm reference
     blocks + m2c body

The Ghidra MCP fetch is the slash command's responsibility, not this script's.
The agent writes `ghidra.c` (the decompile, for shape/types) and, optionally,
`ghidra_ctx.c` (`#include "common.h"` + RE'd struct defs + game externs) so m2c
types the accesses. When m2c fails, base.c falls back to the sanitized Ghidra
body, else a TODO stub.
...
def run_m2c(
    ...
    """Run m2c to generate the seed body. Returns True on success.

    Targets the KMC/GCC MIPS triple (`-t mips-gcc-c`) so the seed matches the
    project's compiler; the default target is IDO and would emit different C.

    `ctx_paths` are preprocessed context files (parent-derived and/or the
    Ghidra-typed struct context); m2c's `--context` is repeatable, so each one
    that exists is passed, letting parent types and RE'd structs both apply.
    """
    cmd = [sys.executable, str(M2C_PATH), "-t", "mips-gcc-c"]
    for ctx in ctx_paths:
...
# A Ghidra decompile is "degenerate" when it is actively wrong rather than merely
# imprecise: a cached `_NON_MATCHING` body, an empty body, or a bare `return
# <const>;` shell. Per docs/hazards.md#decompile-vs-asm-authority the asm is the
# sole authority for these (S11/S152/S156), so the seed routes the agent to the
# inlined asm block instead of trusting the pseudocode. Deliberately conservative
# (clear tells only) so a legitimately terse accessor is not false-flagged.
RETURN_CONST_RE = re.compile(r"^return\s*(?:0[xX][0-9A-Fa-f]+|-?\d+)?\s*;$")


def is_degenerate_ghidra_body(text: str) -> bool:
    """True if the RAW ghidra.c is an untrustworthy `_NON_MATCHING`/empty shell.

    Run on the raw file BEFORE sanitize_ghidra_body, which strips the
    `.NON_MATCHING` suffix and the `/* [MMxx] */` plate that carry the tell.
    """
    ...
    if start < 0 or end <= start:
        return False  # unparseable — don't flag
    ...
    if not body:
        return True
    return bool(RETURN_CONST_RE.match(body))
...
class BaseCSpec:
    ...
    ghidra_path: Path
    missing_rodata: list[str]
    target_s_path: Path | None = None
    ghidra_degenerate: bool = False
    m2c_ok: bool = False
...
def stitch_base_c(spec: BaseCSpec) -> Path:
    ...
    target_s_path = spec.target_s_path
    degenerate = spec.ghidra_degenerate
    m2c_ok = spec.m2c_ok
    lines: list[str] = []
    ...
    # Ghidra decompile reference. Shape/type reference only, never compiled: the
    # m2c body below is the seed. A degenerate `_NON_MATCHING` decompile is
    # suppressed (it is actively wrong, not merely imprecise) rather than shown.
    # See docs/hazards.md#decompile-vs-asm-authority.
    if ghidra_path.exists():
        ...
        if degenerate:
            ...
            lines.append("// Types below come from the m2c body and the externs above.")
        else:
            lines.append("#if 0")
            for gl in safe_read_text(ghidra_path).splitlines():
                lines.append(gl)
            lines.append("#endif")
        lines.append(
    ...
    # Asm ground truth. The `.s` slice (disassemble_function's equal) is the sole
    # authority; both the m2c body and the Ghidra decompile can be silently wrong.
    # Embed it inline so the agent verifies the seed against it.
    # See docs/hazards.md#decompile-vs-asm-authority.
    if target_s_path is not None and target_s_path.exists():
        lines.append(
        ...
        lines.append("#if 0")
        for al in safe_read_text(target_s_path).splitlines():
            lines.append(al)
        lines.append("#endif")
        lines.append(
    ...
    lines.append("// === Function body (SEED — edit during Iterate) ==============")
    if m2c_ok and m2c_path.exists():
        # m2c is the primary seed body: it translated the asm, typed by the
        # Ghidra struct context. Rename its temp_*/local_*/arg_* synthetics and
        # reconcile its extern preamble against the concrete externs above per
        # docs/coding-style.md, then verify against the asm block.
        lines.append("// m2c seed: rename synthetics + reconcile the extern preamble.")
        for ml in safe_read_text(m2c_path).splitlines():
            lines.append(ml)
    elif ghidra_path.exists() and not degenerate:
        # m2c failed: fall back to the sanitized Ghidra decompile as the body.
        lines.append(
            "// m2c failed; falling back to the Ghidra decompile — verify vs the asm."
        )
        lines.append(sanitize_ghidra_body(safe_read_text(ghidra_path), placeholder).rstrip())
    else:
        lines.append(
            f"// TODO: no m2c seed and no trustworthy decompile for {placeholder} —"
        )
        lines.append("// translate from the asm ground-truth block above.")
...
def main() -> None:
    ...
    # m2ctx: parent-derived type context (best-effort)
    ...
    if args.parent:
        ...
        if not parent_path.exists():
            ...
            log(
                ...
                "(no parent type context)"
            )
        else:
            has_ctx = run_m2ctx(parent_path, ctx_path)

    # m2ctx: Ghidra-typed struct context (agent-written ghidra_ctx.c, if present).
    # This is `#include "common.h"` + RE'd struct defs + game externs — the half
    # that lets m2c type the accesses instead of emitting raw offsets.
    ghidra_ctx_src = out_dir / "ghidra_ctx.c"
    ctx_ghidra_path = out_dir / "ctx_ghidra.c"
    has_ghidra_ctx = False
    if ghidra_ctx_src.exists():
        has_ghidra_ctx = run_m2ctx(ghidra_ctx_src, ctx_ghidra_path)
        log(
            f"[seed] ghidra_ctx.c {'preprocessed' if has_ghidra_ctx else 'FAILED to preprocess'}"
        )

    # m2c: the seed-body generator, typed by whichever contexts preprocessed.
    m2c_path = out_dir / "m2c.c"
    ctx_paths = [
        p for p, ok in ((ctx_path, has_ctx), (ctx_ghidra_path, has_ghidra_ctx)) if ok
    ]
    m2c_ok = run_m2c(target_asm, ctx_paths, rodata_path, m2c_path)
    ...
    # Ghidra body (written by slash command, not this script). Trust it only if
    # it is not a degenerate `_NON_MATCHING`/return-const shell; otherwise the
    # seed routes the agent to the inlined asm ground truth.
    ghidra_path = out_dir / "ghidra.c"
    ghidra_present = ghidra_path.exists()
    ghidra_degenerate = ghidra_present and is_degenerate_ghidra_body(
        safe_read_text(ghidra_path)
    )
    if ghidra_degenerate:
        log(
            "[seed] WARN: ghidra.c looks degenerate (_NON_MATCHING/empty/return-const); "
            "routing the agent to the asm ground truth"
        )
    ...
    base_path = stitch_base_c(
        BaseCSpec(
            ...
            target_s_path=target_asm,
            ghidra_degenerate=ghidra_degenerate,
            m2c_ok=m2c_ok,
        )
    )

    # Which artifact the emitted body came from (mirrors stitch_base_c's chain).
    if m2c_ok:
        body_source = "m2c"
    elif ghidra_present and not ghidra_degenerate:
        body_source = "ghidra"
    else:
        body_source = "stub"
    log(f"[seed] {base_path.relative_to(ROOT_DIR)} written (body: {body_source})")
    ...
    emit(
        {
            ...
            "ctx_ghidra_c": str(ctx_ghidra_path.relative_to(ROOT_DIR))
            if has_ghidra_ctx
            else None,
            "m2c_c": str(m2c_path.relative_to(ROOT_DIR)),
            "m2c_ok": m2c_ok,
            "body_source": body_source,
            "ghidra_ctx_expected": str(ghidra_ctx_src.relative_to(ROOT_DIR)),
            "ghidra_ctx_present": ghidra_ctx_src.exists(),
            "ghidra_c_expected": str(ghidra_path.relative_to(ROOT_DIR)),
            "ghidra_c_present": ghidra_present,
            "ghidra_body_degenerate": ghidra_degenerate,
            "asm_inlined": target_asm.exists(),
