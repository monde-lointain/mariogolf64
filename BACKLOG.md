...
**Remaining libultra hazard map (S38; alHeapDBAlloc line corrected S53).** Clean warm mirrors still
available in open bands (`monegi/ai/`, `nintendo/pi/`, `monegi/thread/`). ~~**blk** —
`alHeapDBAlloc`~~ **BANKED S53** — was never actually header-blocked: `libaudio.h`/`os_internal.h`/
`ultraerror.h` all ship at `include/libultra/PR/` and resolve via `LIBULTRA_CFLAGS -I include/libultra/PR`;
the `blk` was a `missing_includes` `-I`-model gap (base INCLUDE_DIRS omits PR/), now fixed via
`LIB_EXTRA_INCLUDE_DIRS` (S53 #1). **Re-survey caveat:** with the PR-band false-blk fixed, `--lib libultra`
is *still* empty — the remaining named-libultra leaves carry real blockers (`sprintf`: file-static +
pack:2fn + genuine `needs-header:xstdio.h,string.h`; the cache ops `osInvalDCache`/`osWritebackDCache*`
are `intrinsic-likely`/upstream-none → classical-or-hasm, not mirrors). So the libultra cheap-mirror
band is genuinely near-exhausted; next libultra progress is a needs-header unblock (`sprintf`) or a
classical leaf, not a hidden clean mirror. **defines-data (classical)** — `__osViInit` (3pt, defines placed BSS globals, static drops
