...
- **`src/libultra/io/contquery.c` (`osContStartQuery`=func_800A7190, `osContGetQuery`=func_800A7210)
  — the asm HEAD of the S74-split `0x82590` subseg; the direct coddog 99.99 hit.** NOT a spike — a
  near-free retry: the tail (contreaddata.c) banked S74, leaving the head as `[0x82590, asm]`. Fully
  pre-scoped: split `[0x82590, asm]`→`[0x82590, c, libultra/io/contquery]`; shares the S74 SI+data
  recover-externs (now placed); needs only 2 MORE func recoveries — `__osPackRequestData`=0x800A7660
  + `__osContGetInitData`=0x800A75AC (both confirmed jal targets in the adjacent controller.c 0x82810
  subseg). Verbatim ultralib `src/io/contquery.c` (VERSION_J), same include adaptation as
  contreaddata (PRinternal/→bare). Pull first at the next gate (smallest clean coddog sibling).
