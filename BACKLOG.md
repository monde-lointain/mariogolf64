...
- **C-mirror next-cleanest leaves buried in gu combined subsegs (S64).** With 103/103 `.c`
  stub-free, the next libultra C mirror is a clean leaf hidden inside a multi-file combined asm
  subseg (now flagged `c-combined:<n>file[…]`, S64 #3):
  - **`cosf` (gu/cosf.c, 0x82F20, ~336B, zero-callee leaf)** — inside the `[0x82B80,asm]` 3072B subseg
    (`pack:5fn[guAlignF=align,func_800A794C=?,cosf=?,guLookAtF=lookat,func_800A7FF0=?]`,
    `c-combined:2file[align|lookat]`). cosf shows `=?` because `gu/cosf.c` defines `__cosf` with
    `#pragma weak cosf=__cosf` (upstream_index keys the defined name, not the ROM alias). Verified
    self-contained: `sinf` (0x800AA730) uses a SEPARATE constant pool (0x800D2568..), no sharing.
    Isolate by splitting the subseg at 0x82F20 (cosf start, 16-aligned) + 0x83070 (guLookAtF,
    16-aligned), leaving align/lookat as asm. **CAVEAT (novel, verify at gate):** cosf's rodata
    constants are NAMED in `ghidra_symbols` (kInvPi/kPiHi/kPiLo/kCoeff1-4/kZero/zOneHalf1/kOneHalf2,
    0x800D2468..0x24B8 contiguous) — unlike lookathil's anonymous pool — so a verbatim mirror's
    `static const` statics may collide with those labels at the `.rodata` carve. weak-alias handling
    is the S58 bcopy `_bcopy=bcopy` pattern.
  - **`sinf` (gu/sinf.c, 0x85B30)** — the clean sibling, mirror once cosf is proven.
  - NB: the align/lookat members of the 0x82B80 subseg are NOT clean mirrors — they carry the
    `calls-unplaced:guNormalize` → `vec3f_normalize` substitution (S61) + a `data-static`; classical
    or substituted-mirror, harder than cosf.
