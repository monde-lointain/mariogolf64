...
**Next-cleanest classical candidates (post-perspective, by pts):** `osGbpakCheckConnector` (pts 3,
1fn, needs-header:controller.h vendorable *only* — likely the smallest near-clean classical);
`__osContRamRead`/`__osContRamWrite` (pts 3, jal-mismatch 22-23vs10 → classical, unplaced SI
callees); `guRotateF` (pts 5, 2fn pack, data-static + calls-unplaced).
