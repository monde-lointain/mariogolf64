...
- [ ] **`osAiSetFrequency`** (0x7EEC0, 288B, libultra `monegi/ai/aisetfreq.c`) — `refs-unplaced:osViClock@D_800C9468` (size:0x4) + rodata constant `D_800D22A0` unplaced; `jal-count-mismatch:2vs0` (IO_WRITE macro, expected isolation artifact). Blocker: osViClock and the rodata constant must be placed in `symbol_addrs.txt` before the subseg can be flipped. Stays `[0x7EEC0, asm]`. (Carried from S37)
