...
- [ ] **`osAiSetFrequency`** (0x7EEC0, 288B, libultra `monegi/ai/aisetfreq.c`) — **`.rodata` layout conflict; classical loop required.** subseg already flipped to `[0x7EEC0, c, libultra/monegi/ai/aisetfreq]`; `osViClock=0x800C9468` already in `symbol_addrs.txt`; `-G 0` assembler fix applied (S38). Layer 1 (`.lit4`/`.lit8` link error) = RESOLVED. Layer 2 (blocker): compiler generates 2^32d as `.rodata`, linker places it at 0x800CA270 instead of 0x800D22A0 (owned by `[0xAD5E0, rodata]` asm subseg) — verbatim mirror permanently blocked. **Next sprint: classical loop; write C that avoids the compiler-generated `.rodata` entry for the 2^32 double** (express the u32→float conversion so GCC uses `lui/mtc1` inline for 2^32d rather than a memory load). `IO_WRITE` isolation artifact still expected (score ≠ 0; proof = in-tree byte cmp + make). (Carried from S37+S38)
