...
**`--lib libultra` is now genuinely EMPTY.** The remaining libultra-range leaves are all
`intrinsic-likely` register/FPU shims (`osGetCount`, `sqrtf`, `__osGetCause`, `__osGetSR`,
`__osSetCompare`, `osWritebackDCacheAll`, `func_800ACCC0`) → classical-or-hasm, NOT mirrors. The
libultra cheap-mirror band is fully exhausted; next libultra progress is a classical/hasm decision on
the intrinsic shims, otherwise pivot the scope to libnusys/libkmc fillers at the next gate.
