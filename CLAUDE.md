...
| `intrinsic-likely:<tu>.s(asm-mirror-jtbl:<head>)` (S91: the vendorable .s defines a SYMBOLIC-pointer table — a switch jtbl / fn-ptr `.word <label>` block — the active `.text` references → asm-mirror is a SPIKE, NOT an S84 strip-and-rename: the table extracts to a separate blob with vestigial `.text`-label refs + can't be carve-placed) | #asm-mirror-vendoring |
