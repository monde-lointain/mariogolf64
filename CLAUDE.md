...
- **`/sprint-plan [scope]`** — Scrum plan gate. Reads `BACKLOG.md` + the roadmap rollup, proposes one small increment (one file → md5-candidate, or a cohesive subseg cluster) + its enablers, gets PO approval, then **performs + validates the flip** (agent edits the yaml subseg + `symbol_addrs.txt`, runs `make extract && make` for the green ROM), and writes `SPRINT.md`. Then chains the per-function `/decomp` runs autonomously. **MCP-lock-free.**
- **`/sprint-review`** — Scrum review gate. Verifies the ROM-SHA-1 DoD on the increment, takes PO scope sign-off, and is the **single apply-point** for the sprint's buffered workflow-improvement edits; appends the `RETRO.md` digest + `BACKLOG.md` carry-overs. **MCP-lock-free.**

Only one of `/decomp`, `/decomp-lead`, or `/sync-names` may run at a time — they share the Ghidra MCP slot via `tools/mcp_lock.py` (machine-global directory mutex at `~/.cache/mariogolf64/mcp.lock/`, 30-min stale-TTL; the lock is shared across git worktrees of the same repo). `/sprint-plan` and `/sprint-review` are the two **MCP-lock-free** gates — they never acquire the slot, so they can run while a `/decomp` chain holds it.

## Scrum operating model (lightweight overlay)

The tactical match-loop runs inside a thin Scrum cadence (McConnell, *More Effective Agile*). These are the only ceremonies — keep it lightweight. Artifacts: `BACKLOG.md` (PO-owned ordering rationale + enablers + carry-overs), `docs/decomp_roadmap.md` (auto-generated live candidate list, `/decomp-lead`), `SPRINT.md` (gitignored ephemeral board + resume surface), `RETRO.md` (consolidated digest).
...
- **Cycle.** `/sprint-plan` (plan gate: PO approves a goal + small backlog; the agent then performs + validates the flip enablers) → autonomous execution → `/sprint-review` (review gate: DoD verify, scope sign-off, retro) → repeat. Both gates are MCP-lock-free.
- **Execution model.** Between the gates the agent **chains the per-function `/decomp` / `/decomp-libupstream` runs via the Skill tool in one session, no per-function PO stop**. Each run acquires/releases the MCP lock itself, so chaining is serial and safe. `SPRINT.md` is the resume surface if the middle spans context windows.
- **Sprint = one `src/<seg>.c` file to md5-candidate (0 `INCLUDE_ASM` stubs), or one cohesive subseg cluster** when the file is part of a multi-file pack. The **function is the task** (the `/decomp` unit); the **file/cluster is the increment**. **Cap small early — 1 upstream file, or ≤ ~3–4 functions** — so the middle fits a context window and to calibrate the still-unproven classical loop (every win to date was an upstream mirror). Span multiple files only when **siblings**. Review fires when every committed item is **banked or spiked/carried**.
- **Definition of Ready (gate into a sprint).** Subseg flippable (not `hasm`); coarse size known; upstream-mirror availability noted (yes/no); hazards flagged (file-scope `static` → `/decomp` not `-libupstream`; BSS-layout-conflict; subseg-alignment). **Enablers** (subseg flip + `make extract`, multi-file split, `symbol_addrs.txt` additions) are performed by the **agent at the plan gate** — after the PO approves the goal/scope — and **validated there** (`make extract && make` must still produce the green baserom ROM with the new stubs) before DoR is "ok". This gate build-check is **load-bearing**, not a formality — it surfaced a missing-`mips-linux-gnu-cpp` toolchain regression on 2026-06-11 that was silently emptying every asm object; never skip it. `/sync-names` is **gate-only**, never mid-sprint (a rename breaks in-flight links until `make extract && make`). Execution is then flip-free and autonomous — all yaml/`symbol_addrs.txt` edits land at the gate, so the middle touches only `src/`.
- **Definition of Done (binary; the oracle already exists).** Per-function = the existing `/decomp` finalization (score 0 + byte `cmp` spot-check + inlined + clang-format + full `make` → `build/mariogolf64.z64: OK`, SHA-1 == baserom + committed). Per-sprint = every fn in the file inlined (0 stubs = md5-candidate) + ROM SHA-1 matches + committed. **The ROM SHA-1 is green at every commit** (un-decompiled parts fill from extracted asm via the Makefile `md5sum -c`), so at review the `make`+SHA-1 paste is a **regression guard**; the *value* delta is the matched-count / md5-candidate-files number. Never commit a non-matching fn; the global md5 makes a separate per-unit regression check unnecessary.
...
- **Resume protocol (multi-session).** On a fresh mid-sprint session: read `SPRINT.md` + `docs/decomp_roadmap.md`, reconcile banked work via `git log` since the snapshot, check the MCP lock holder (`tools/mcp_lock.py`), and verify Ghidra MCP connectivity (`list_instances`, port 8089) before any `/decomp` run.
...
- **Workflow retrospection**: every `/decomp` / `/decomp-libupstream` run ends with a numbered "Suggested workflow improvements" section in the agent's final chat response. **Inside a sprint** (a `SPRINT.md` exists), these are *recorded* into `SPRINT.md`'s suggestion buffer and applied **only at the `/sprint-review` gate** — never mid-sprint — so the sprint's matching behavior stays fixed and its measurement stays clean (see `## Scrum operating model`). **Outside a sprint** (ad-hoc `/decomp`), the old behavior holds: the user replies with which numbered items to apply and the agent edits CLAUDE.md / seed_c.py / decomp_loop.py / decomp.md within the same session.
...
- **Forbidden agent edits**: `ghidra_symbols.txt` (hand-edit forbidden; modify only via `make sync-names` or `/sync-names`), `mariogolf64.ld`, `re_tracking/*.yml`, `reloc_addrs.txt`. **Agent-editable** (boundary moved 2026-06-11): `mariogolf64.yaml` subseg flip/split/path-qualifier lines only (never segment/memory-map structure), and `symbol_addrs.txt` add-only (never delete; keep disjoint from `ghidra_symbols.txt`).
