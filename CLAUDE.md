...
- **`.rodata` sibling yaml pattern (compiler-generated rodata at a different ROM offset than `.text`)**: when GCC emits a `.rodata` constant for a C file whose text is at one ROM offset but whose `.rodata` must land at a different ROM offset (e.g. `osAiSetFrequency`: text at 0x7EEC0, 2^32d double at 0xAD6A0), add a **dot-prefixed** `[0xRODATA_ROM, .rodata, path/to/file]` subseg at the correct ROM offset with the **same name** as the `c` subseg. Splat matches siblings by name; the dot-prefix subseg's `out_path()` returns `sibling.out_path()` (the compiled C object), placing `build/src/xxx.o(.rodata)` at the correct VRAM. `auto_link_sections` finds the explicit sibling already in the list and does **not** insert a duplicate `.rodata` entry at the text-section yaml position. `should_self_split()` = False for dot-prefix types — no asm file extracted. The adjacent autogenerated asm subseg may need to be split manually so the `.rodata` subseg covers only the correct ROM bytes (e.g. split `[0xAD5E0, rodata]` at 0xAD6A0 before inserting `[0xAD6A0, .rodata, …]`). S38: `[0xAD5E0, rodata]` split at 0xAD6A0; `[0xAD6A0, .rodata, libultra/monegi/ai/aisetfreq]` (0x10 bytes = 8-byte double + 8-byte pad) places `aisetfreq.o(.rodata)` at 0x800D22A0 ✓.
- **IO_WRITE/IO_READ isolation artifact (MMIO functions)**: functions that write/read N64 MMIO registers via `IO_WRITE`/`IO_READ` macros always score ≠ 0 in isolation — the macros expand to `*(vu32*)<literal_addr>` (e.g. `0xa4500000`), baking in constants; the reference asm object uses symbolic relocations (`%hi(AI_DRAM_ADDR_REG)`, `%hi(AI_LEN_REG)`) that assemble to placeholder zeros. asm-differ sees mismatched `lui` immediates (`lui a1,0xa450` vs `lui a1,0x0`) — up to 600-point penalty with `total_rows == match_count` and `top_mismatches: []`. No C edit can fix this; it's isolation-only. Correct path: verify the C logic matches the asm instruction-by-instruction (will match if upstream is correct), **inline the body, run in-tree byte `cmp` spot-check, then `make` + ROM SHA-1**. Do not waste iterations. S34: `osAiSetNextBuffer` — score=600 throughout, spot-check MATCH.
...
- **Spot-check via byte-level `cmp` only**: `mips-linux-gnu-objdump` renders both `0x...1021` and `0x...1025` as `move v0,zero`, so mnemonic-level diff silently false-positives. Always `cmp` raw `.text` bytes. **Split-subseg cmp target**: when a subseg was split at a new offset (e.g. `0x7E350` split from `0x7E330`), `decomp_loop.py` resolves the function to the OLD pre-split segment label and references `build/asm/7E330.o` — which contains multiple functions and gives a misleading mismatch. Use `build/asm/<subseg_hex_offset>.o` for the specific subseg containing the target (e.g. `build/asm/7E350.o`), not the combined parent object. **Base-symbol struct-field reloc-encoding artifact (classical libultra only)**: when a C file accesses struct fields via a base symbol (e.g. `vi[1].retraceCount` via `lui at, %hi(vi); sh v0, struct_offset(at)`), the compiled object encodes the struct offset as an inline LO16 addend (`sh v0, 0x32(at)`) while the reference asm object uses a zero-placeholder for the per-field label (`sh v0, 0x0(at)` + `R_MIPS_LO16 D_800C9532`). The byte-level `cmp` fails on these addend bytes, but both encodings resolve to identical instructions after linking — this is a linking-equivalent isolation artifact. When the only `cmp` differences are inline addend bytes on `sh`/`sw`/`lw` struct-field stores/loads and/or IO_WRITE literal addresses, proceed directly to `make` + ROM SHA-1 without C iteration.
...
- **Near-verbatim mirror (upstream-vs-ROM call divergence)**: this ROM's build can disagree with the upstream `.c` on which calls it makes. S18 `nuContInit`: upstream calls four managers (`nuSiMgrInit`/`nuContMgrInit`/`nuContPakMgrInit`/`nuContRmbMgrInit`), but the ROM's asm has only **three jals** — `nuContPakMgrInit` is absent from this build. A verbatim copy would emit the extra call and never match. This stays a **mirror**, not a classical loop: copy upstream verbatim, then **drop the diverging line** (deterministic from the asm — disassemble the target fn and reconcile its jal list against the upstream call list), and the proof is still the full-`make` ROM SHA-1 (no iteration). Like the file-scope-`static` drop, it's a known-edit mirror sub-case. `tools/pick_target.py` surfaces it as the **`jal-count-mismatch:<C>vs<asm>`** hazard (advisory — it strips dead `#ifdef _DEBUG`/`#ifndef _FINALROM` blocks first, but macro/inlined calls can still skew the count, so the gate confirms by disassembling). The S16 gate asm-data-recovery check generalizes to this: before declaring a clean verbatim mirror, reconcile the upstream call list against the ROM's actual jals. **Large mismatch is `classical-likely`**: when the jal count difference is >2 (e.g. `5vs2`), the ROM may implement fundamentally different logic — not just dropping calls, but different branches, different arguments, or stripped control structures — rather than a near-verbatim drop. At the gate, if `jal-count-mismatch` is flagged AND the difference is >2, disassemble the target fn and compare asm logic structure vs the upstream body before routing to the mirror branch; a large mismatch should default to the classical loop unless the disassembly confirms a clean line-drop (S30: `osSetTimer` `5vs2` was a stripped classical, not a 3-call drop). **Small mismatch (≤2) — try verbatim first**: when the difference is ≤2 and the upstream has multiple call sites with identical function + args across separate switch branches or if-branches, the mismatch may be a **GCC -O3 tail-merge artifact** — the compiler shares one `jal` instruction for all paths (S35: `osStartThread` `9vs7` = three `__osEnqueueThread(&__osRunQueue, t)` sites in two switch cases merged to one jal). In this case a verbatim copy compiles correctly with no drops. Correct approach: try verbatim copy + `make` first; only investigate line-drops if the ROM SHA-1 doesn't match.
- **Recover-extern mirror (the third known-edit mirror sub-case)**: a clean verbatim leaf often references a global the upstream declares `extern` (defined in not-yet-decompiled lib code) that is absent from both name files — `tools/pick_target.py` surfaces it as **`refs-unplaced:<g>@0x<ADDR>`** with the vram inlined. The gate flow is fully deterministic, **zero iteration**: (1) confirm the vram from the target fn's own `lui/addiu` HI/LO16 pair via MCP `disassemble_function` (do **not** skip this — **mandatory**; **to address the `disassemble_function` call, look up the *target fn's* own vram — never guess a flat `rom + <round constant>` offset**: if its curated name is already in `ghidra_symbols.txt`/`symbol_addrs.txt` read the vram from there (authoritative), else derive it from the yaml `main` segment base (`vram 0x80025C00 @ rom 0x1000` → `vram = rom + 0x80024C00`). S22 caught a flat guess (`rom + 0x80020000`) that resolved *mid-function* and silently returned a different ~1000 B containing fn — only the size mismatch vs the expected 80 B leaf flagged it. As for the recovered extern's vram: the inlined vram matched the asm S12 + S19's 3/3, but **S20 was the first miss**: for an *indexed-struct-array* extern (`base[i].member`) the inlined `refs-unplaced` vram is the **field-access** address, not the array base — recover the base by subtracting the `lbu/lw` displacement's struct-member offset from the materialized address (`nuContRmbCtl`: inlined 0x80104F57 = base + offsetof(`.mode`)=7 → base 0x80104F50). The re-confirm is the only thing that catches this), (2) add `<g> = 0x<ADDR>; // size:0x<n>` to `symbol_addrs.txt` as a **data extern** (no `type:func`; **size rule**: a scalar / function-pointer / word global is `size:0x4`; an **array** global is `size:0x<stride×count>` — stride from the index multiply in the asm, count from the index bound / `MAXCONTROLLERS` — e.g. `nuContRmbCtl` 0x28 = 10×4, `nuContPfs` 0x1A0 = 0x68×4), add-only and disjoint from `ghidra_symbols.txt`, (3) flip the subseg, verbatim `cp` the upstream, (4) full-`make` → ROM SHA-1 is the proof. This is the **lowest-risk mirror** and **batches well**: S19 banked the `nuGfx*FuncSet` trio (`nuScPreNMIFunc`/`nuGfxFunc`/`nuGfxSwapCfbFunc`, one extern each) in a single 3-file sprint at one-file risk. Distinct from `defines-data` (the leaf *defines* a placed global → BSS-layout-conflict → classical loop with the def dropped); a *referenced* extern is safe to place. **Function-callee dual (S23)**: the same recovery applies to an unplaced *function* the leaf **calls** — a callee labelled `func_<addr>` in the asm (absent from both name files) has no `<name>` symbol for the verbatim C reference to bind, so the mirror **link-fails** once the body calls it by name (S23 `osEPiStartDma` → `osPiGetCmdQueue`@0x800B06F0). Critically, **the gate build-check cannot catch this** — the `INCLUDE_ASM` scaffold resolves the jal directly, so the missing symbol only bites in the execution middle. `tools/pick_target.py` now surfaces it as the **`calls-unplaced:<fn>@0x<ADDR>`** hazard (vram inlined when the binding is unambiguous — one unplaced call ∩ one `func_<addr>` jal), the function dual of `refs-unplaced`. Recovery is identical and deterministic: the callee's vram **is** its jal target; add `<fn> = 0x<ADDR>; // type:func` to `symbol_addrs.txt` (add-only) at the gate, BEFORE the flip. So before declaring a clean verbatim mirror, **reconcile the full callee list — data *and* functions — against the name files** in the same disassemble pass that confirms the recover-extern vram (the asm jal list is already in hand). The flag is comment/string/dead-block-stripped but heuristic (a function-like macro can over-flag) — the gate confirms against the asm jals.
- **Macro-hidden recover-extern (S41)**: a recover-extern (data global *or* callee) can hide inside a **library macro** the leaf invokes rather than appearing in the `.c` body — so neither the `.c`-source ref-grep nor the gate build-check sees it, and the mirror **link-fails in the execution middle** (same blind spot as `calls-unplaced` and the wrong-library std-header). S41 `__osEPiRawWriteIo` looked zero-enabler (symbol pre-placed, no `.c`-body data refs), but the `EPI_SYNC` macro in `piint.h` references `__osCurrentHandle[domain]` → `undefined reference to __osCurrentHandle` at link. Recovery is the **identical deterministic flow** as the plain recover-extern: read the unplaced global's vram from the target fn's own `lui/addiu` HI/LO16 pair in the extracted asm (`__osCurrentHandle` = `D_800C7E90` materialized by `lui $a3,%hi(D_800C7E90)` + `lw $a3,%lo(D_800C7E90)($a3)`; the index `domain*4` is a *separate* `addu`, so the `%hi/%lo` base is direct — no field-offset subtraction), add `<g> = 0x<ADDR>; // size:0x<n>` to `symbol_addrs.txt` (`__osCurrentHandle` = an `OSPiHandle*[2]` array per `extern … __osCurrentHandle[2]` → `size:0x8`), `make extract && make` → ROM SHA-1 is the proof. **Prevention**: when mirroring a leaf that invokes a library macro (`EPI_SYNC`, `EPI_*`, `WAIT_ON_IOBUSY`, …), expand the macro and reconcile **its** referenced globals/callees against the name files in the same disassemble pass — don't trust the `.c`-body ref-grep alone. `tools/pick_target.py` now does this automatically: `refs-unplaced`/`calls-unplaced` **follow one level of macro expansion** (the directly-invoked macros' bodies, with each macro's own parameter names stripped and `__builtin_*`/nested macro names excluded), so a macro-hidden recover-extern surfaces at the gate as a normal `refs-unplaced`/`calls-unplaced` flag instead of a mid-execution link failure (still gate-confirmed against the asm).
...
- **libkmc compile profile is `-O`, libultra is `-O3 -funsigned-char`**: libkmc upstream was built with `gcc -O` (per `~/development/repos/libkmc/src/genn64.bat`), not `-O2`. The instruction scheduler differs between the two: at `-O` rand.c stores `next` between the two `addiu` ops; at `-O2` it gets pushed to the end. Same C, same compiler, different `-O` level → byte mismatch. The Makefile carves this via `LIBKMC_CFLAGS := $(subst -O2,-O,$(CFLAGS))` and a `$(BUILD_DIR)/$(SRC_DIR)/libkmc/%.o` pattern rule whose specificity beats the generic `src/%` rule. **If a libkmc match locks at ~0.9 percent across every C rewrite, the diff is almost certainly scheduler-level — verify by compiling `nonmatchings/<func>/base.c` with `-O` directly before iterating further on the C.** `decomp_loop.py` auto-applies the `-O` profile when a matching `src/libkmc/<basename>.c` exists for the target's segment. libultra upstream was built with `OPTFLAGS=-O3 MIPS_VERSION=-mips3 -funsigned-char` for VERSION_J (ultralib gcc.mk). The global CFLAGS uses `-mips3` (changed from `-mips2` in S33 based on this discovery). The Makefile carves `LIBULTRA_CFLAGS := $(subst -O2,-O3,$(CFLAGS)) -funsigned-char -DBUILD_VERSION=VERSION_J` and a `$(BUILD_DIR)/$(SRC_DIR)/libultra/%.o` pattern rule. The `-O3` level enables inlining of small same-TU functions and affects delay-slot scheduling. **If a libultra match locks at ~0.9 percent with unexpected register usage or wrong delay-slot instructions, verify the loop is using `LIBULTRA=1` (or `--profile libultra`) before iterating on the C.** `decomp_loop.py` auto-applies the libultra profile when a matching `src/libultra/<relpath>.c` exists for the target's segment (same auto-detect logic as libkmc, but using `rglob` over the ultralib upstream).
- **Per-library standard-C-header isolation (libultra std headers; `lldiv_t` S40)**: libultra and libkmc each ship their own standard C headers (`stdlib.h`, …) — they are NOT interchangeable (libultra's `stdlib.h` defines `lldiv_t`; libkmc's lacks it). The global CFLAGS `-I` order puts `include/libultra` before `include/libkmc`, but **neither dir holds the std headers directly**, so a bare `#include "stdlib.h"` from *any* source falls through to `include/libkmc/stdlib.h`. A verbatim libultra mirror that needs a libultra-only type (S40 `ldiv.c`→`lldiv`→`lldiv_t`) therefore link/compile-fails against the libkmc header. **Fix in the build system, never by adding the type to the libkmc header** (that pollutes a verbatim libkmc mirror and defeats cross-referencing — same rule as "never clang-format library code"). Vendor the libultra std header **verbatim** from `ultralib/include/compiler/gcc/` (KMC GCC 2.7.2 → the `gcc` variant; `lldiv_t` is identical across the gcc/modern_gcc variants) to `include/libultra/compiler/gcc/`, and **prepend** `-I include/libultra/compiler/gcc` to `LIBULTRA_CFLAGS` only — so it wins over `include/libkmc` for libultra sources while libkmc/libnusys/other sources keep resolving to the libkmc copy (the libultra-only `-I include/libultra/PR` is the precedent). Verify per-file with `tools/cc/gcc -M -nostdinc <profile -I set> src/…`: the libultra source must resolve to `include/libultra/compiler/gcc/stdlib.h`, the libkmc source still to `include/libkmc/stdlib.h`. **Pick/DoR blind spot**: `pick_target.py`'s `needs-header` greps include *resolvability*, not *which library's* header resolves nor whether it has the needed types — a resolvable-but-wrong-library include passes the grep AND the gate build-check (the `INCLUDE_ASM` scaffold never compiles the C body), surfacing only in the execution middle (like `calls-unplaced`). When mirroring a libultra leaf that includes a bare standard header, confirm at the gate that the libultra std header for it is vendored.
- **`make sync-names` eviction recovery (mid-sprint build break)**: running `make sync-names` mid-sprint can evict symbols from `ghidra_symbols.txt` — either because Ghidra doesn't have a curated name for them (they vanish entirely) or because a function was renamed (old spelling disappears, new spelling appears). Any in-flight C code that referenced the old symbol by name will fail to link. Diagnosis: if `make` fails with `undefined reference to <sym>` and that symbol was in `ghidra_symbols.txt` before the sprint, check whether `make sync-names` was run mid-sprint. Recovery: add the missing symbol to `symbol_addrs.txt` as a data extern (`<name> = 0x<ADDR>; // size:0x<n>`) — add-only, disjoint from `ghidra_symbols.txt` — then re-run `make extract && make`. This restores the symbol via the splat-side file rather than Ghidra. **Prevention: `make sync-names` is gate-only (see the DoR rule above); never run it mid-sprint.**
- **Stale top-level asm label sync after a gate rename**: when `make sync-names` renames a function (e.g. `__osPiRelAcces` → `__osPiRelAccess`), `make extract` regenerates the per-function stub in `asm/nonmatchings/...` with the new name — but does **not** update the stale top-level `asm/<seg>.s` (which is no longer regenerated once the subseg is c-flipped). `tools/decomp_loop.py` uses the top-level asm file for `find_segment`, so the old label breaks loop resolution ("no `glabel <newname>` found"). Fix: manually update the old label(s) in the stale `asm/<seg>.s` to the new curated name, then rebuild the reference object (`make build/asm/<seg>.o`). Only 3 occurrences per function (`nonmatching`, `glabel`, `endlabel`).
- **Stale top-level asm label sync after a `symbol_addrs.txt` add (data-global analogue)**: when a BSS global is added to `symbol_addrs.txt` (e.g. `hdwrBugFlag = 0x800C7EC0; // size:0x1`), `make extract` regenerates the nonmatchings stub with the new name — but does **not** update the stale top-level `asm/<seg>.s` (no longer regenerated once c-flipped). `tools/decomp_loop.py` builds the reference from `asm/<seg>.s`; if it still uses the auto-generated label (`D_800C7EC0`) while the compiled object uses the new name (`hdwrBugFlag`), asm-differ sees a relocation-name mismatch and penalises every load/store to that global — masking the real score. Fix: replace all occurrences of the old `D_<ADDR>` label in `asm/<seg>.s` with the new name, then rebuild (`make build/asm/<seg>.o`). S34: `asm/7EFE0.s` — 3 occurrences of `D_800C7EC0` → `hdwrBugFlag`.
