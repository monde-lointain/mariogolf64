...
Paste the real command output and claim nothing the commands don't show (grounding guard against a
...
- **Progress delta:** matched count + md5-candidate files now vs the `SPRINT.md` snapshot (the
  snapshot + `git log`, with `venv/bin/python3 tools/pick_target.py` for the live count).
...
- **Quality counter-metric:** stuck-far + permuter-escalated + carried + re-opened, read from the
  `SPRINT.md` standup log — the balance metric so the count isn't gamed by premature spikes.
- **Story points:** the committed seed from `SPRINT.md ## Estimate`, with per-file banking (a spiked
  file scores 0 pt; a banked sibling still counts). For a **mirror** sprint, log seed-only — the
  mirror track is a point mass: `committed <N>pt; banked <B>pt; regime mirror`. For a
  **classical/mixed** sprint, also score the v2 realized tier + residual on the classical track
  (active since Sprint 11): start at the seed band, +1 per {stuck-far / permuter / re-attempt /
  novel gotcha / mid-sprint split / carry-or-reopen}, −1 for a verbatim first-try; report
  `seed <S>; realized <R>; residual <R−S>; regime <classical|mixed>`. See `VELOCITY.md`.
...
If the PO approved push/PR, do it now (and only now). Leave `SPRINT.md` for the next `/sprint-plan`
to overwrite. Suggest running `/sprint-plan` for the next sprint.

Optionally close with a short numbered "Suggested Scrum-process improvements" list (about the
overlay itself — gates, artifacts, cadence). These buffer the same way matching suggestions do and
are applied at the next review.
...
- Increment: src/libultra/nintendo/pi/epirawwrite.c banked (1 fn). matched 312→313; md5-candidate 47→48.
...
- What helped: macro-hidden recover-extern surfaced at the gate via pick_target.py's 1-level macro
  expansion (EPI_SYNC → __osCurrentHandle), so no mid-execution link failure.
- Friction: none.
