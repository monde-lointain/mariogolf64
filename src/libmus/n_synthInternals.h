...
/*
 * n_audio DMEM scratch layout, as byte offsets. The early-pipeline buffers
 * alias offset 0 (decoder-in, resampler-out and temp 0 all share it, so decode
 * and resample run in place); the decode/temp blocks sit one 368-byte
 * (184-sample s16) block apart, and the main/aux L+R mix outputs follow above
 * them.
 */
...
typedef struct N_PVoice_s {
  ...
  s32 dc_lastsam; /* samples consumed so far in the 16-sample ADPCM frame */
  s32 dc_first;   /* first-block flag */
  s32 dc_memin;   /* DRAM read address for the next ADPCM frame */
  ...
  ALParam* em_ctrlTail; /* tail of the ctrlList queue */
  s32 em_motion;        /* playback state (AL_PLAYING / AL_STOPPED) */
  s32 offset; /* voice-steal fade offset, in samples (0 = not stealing) */
} N_PVoice;
...
/* Main bus: the single n_audio filter that produces the final stereo output. */
