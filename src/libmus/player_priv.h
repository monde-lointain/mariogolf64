...
extern musHandle __MusIntFindChannelAndStart(fx_header_t*, int, int, int, int);
...
extern void __MusIntRemapPtrs(void*, void*, int);
...
extern int __MusIntFifoAddCommand(fifo_t*); /* enqueue; 0 if the ring is full */
...
extern void __MusIntHandleSetFlag(unsigned long, unsigned long, unsigned long);
...
extern void __MusIntRemapPtrBank(char*, char*);
...
extern int __MusIntFindChannel(song_t*, int);

/* Zero a channel and load its default state (volumes, envelope, bank). */
extern void __MusIntInitialiseChannel(channel_t*);
...
extern void __MusIntProcessContinuousVolume(channel_t*);
extern void __MusIntProcessContinuousPitchBend(channel_t*);
